...
void * getFibbo(void *data_void_ptr){
    data *data_ptr = (data *)data_void_ptr;
    ...
    pthread_mutex_lock(data_ptr->bsem);
    ...
    pthread_mutex_unlock(data_ptr->bsem);
    return NULL;
}
...
int main(int argc, char *argv[]){
    ...
    for(int i = 0; i < myData.size(); i++){
        ...
        upon creation, execute the getFibbo function with &sortedData[i] as argument*/
        if (pthread_create(&threads[i], NULL, getFibbo, &myData[i]))
		{
